...
exports.Init = async (media, payload, outputLocation) => {
  if (!media) {
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      for (let j = 0; j < seasons.length; j++) {
        ...
        while (episodesMoreBelow) {
          const parsedEpisodesData = await getEpisodesAndParse(seasonId, episodesOffset);
          if (parsedEpisodesData.length === 0) {
            const newParsedEpisodesData = await getEpisodesAndParse(seasonId, episodesOffset, { listType: 'useful', includePreOrders: false });
            if (newParsedEpisodesData.length !== 0)
              episodes.push(...newParsedEpisodesData);
          } else {
            episodes.push(...parsedEpisodesData);
          }
          if (parsedEpisodesData.moreBelow) {
            episodesMoreBelow = parsedEpisodesData.moreBelow[0] === 'true';
          } else {
...
const getEpisodesAndParse = async (seasonId, episodesOffset, option) => {
  try {
    const episodesURL = getEpisodesURL(seasonId, episodesOffset, option);
